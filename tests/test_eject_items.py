...
def _what_happens(pos: Position, suspended: bool) -> str | None:
...
def test_the_ejected_holders_move_goes_with_it(reg, oracle: Oracle) -> None:  # noqa: ANN001
    ...
    for pause in result.suspended:
        ...
        for branch in resumed.branches:
            ...
            assert mon.species == "kingambit" and not mon.boosts, mon.boosts
...
def test_the_port_switches_the_holder_as_showdown_does(reg, oracle: Oracle, name: str) -> None:  # noqa: ANN001
    ...
    result = resolve_turn(reg, before, _actions(reg, before, choices), budget=BUDGET)
    outcomes = [(b.position, False) for b in result.branches]
    outcomes += [(s.position, True) for s in result.suspended]
    assert outcomes
    if what == "eject":
