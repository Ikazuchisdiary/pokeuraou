...
> **9/24（IKA-228）からこのファイルは凍結。** 新しい記録は `records/IKA-NNN.md`（1 課題 1 ファイル）と `records/YYYY-MM-DD-<名前>.md`（課題をまたぐもの）に書く。置き方は `records/README.md`。ここにある節（「9/24 — IKA-NNN」など）への参照はそのまま有効で、この先も書き換えない。
...
## 9/24 — IKA-223: 控え隠蔽の交代の行列の番号ずれ（9/17〜IKA-209）が教材に及んだ範囲 —— 交代の決定の 7 割（学習の行の 16〜18%）で行列が動いたが、動いたのは記録の値（|Δ| 平均 0.004〜0.017）だけで、方策が変わったのは 1,686 決定中 11。学習が読む項目（局の勝敗・本当の局面）は動いていない → (a) 放置でよい

ブランチ `ika-223-replacement-index-audit`（master bd5deb4 から）。rust/src・src/pokeuraou は触っていない。道具 `tools/replacement_audit.py`（新）。
一時ファイルは `C:/tmp/ika223/`（走らせ方は `run_count.sh`・`run_solve.sh`・`run_checks2.sh`）。

答えた問いは次の 4 つ。
「どの教材の、どれだけの決定に誤りが届き得たか」（§1）
「届いた決定で、記録の方策・値は正しい答えからどれだけずれていたか」（§2）
「学習はそのずれを読むか」（§3）
「深さ 2 の比べの数字に規則の差の注記が要るか」（§5）

### 0. 誤りの形（IKA-209 §2 (a) の再掲と、道具での再現のしかた）

d959ab0（9/17 19:45）から IKA-209 の着地（5d54631、9/24 12:22）まで、控え隠蔽の交代の決定は相手の見えていない控えの完成形ごとに行列を作り、
Python の `resolve_replacements` は `switch_to=party_index`（1 始まり）を渡していた。`_find_switch_target` は種族で先に探すので、
本当の局面では当たる。完成形で、選択肢が名指す種族がその完成形に居ないときだけ番号に落ち、**次の番号の控え**（範囲外・瀕死・場に居る
なら誰も出ない）を出していた。**打った交代は本当の局面で解いたので正しい。ずれたのは記録の `ownPolicy`・`foePolicy`・`searchValue`・
`foeSearchValue` と、両席がそこから引いた交代の選び方。**

port の `switch_target`（commands.rs）と `do_switch_with`（resolve.rs）は Python と同じ「種族、次に番号」で、番号は `party_index - 1`。
だから**選択肢の `party_index` を 1 つ上げて port に渡すと、記録した時の Python の落ち方がそのまま再現する**（道具の `shifted`）。
上げないものが今の正しい答え（`fixed`）。

### 1. 教材ごとの数（`replacement_audit.py count`、全局、heavy.py --cores 8、壁時計 5 s + 10 s）

d959ab0 を含むかは、全局の `engine.commit` を `git merge-base --is-ancestor d959ab0 <commit>` で見た（各教材の中で commit は 1 つだけ）。
worker log の echo（葉・選出・隠蔽）とも突き合わせた。

「効いた可能性のある決定」= 控え隠蔽の局の交代の決定で、ある席 t が交代を負い、t の選択肢の 1 つが t の**見せていない**控えへの交代で、
t のシートの候補が見えていない枠の数より多い（= その種族を含まない完成形がある）もの。そのとき相手席（1−t）の行列のセルが動く。
「見せた」の数えは、その時の生成の規則に合わせた: IKA-117 より前は番号を持ち越す（`slots`）、g600 は記録の `shownIdentities`。
場の種族は 43867b8 より前は `species` だけ（メガの元の種族が候補に残る旧い形）、以後は `shown_species`。
§2 の標本では、この数えで「届く」とした 1,686 決定の**すべて**で、少なくとも 1 セルが正の信念の重みで実際に動いた（`any_moved` = n）。

```
  教材                        engine の commit（日時）          d959ab0  局       決定       うち交代   自交代   効いた可能性   交代に対し   学習の行に対し
  selfplay-gen11L             e9488d5（9/17 21:09）             含む     12,000   160,495    36,621     5,856    26,931        73.5%        16.8%
  selfplay-hidden2            affb931（9/19 05:08）             含む     12,070   163,110    36,700     6,448    26,163        71.3%        16.0%
  selfplay-hidden-fixed       43867b8（9/19 19:09）             含む        600     8,099     1,839       318     1,320        71.8%        16.3%
  selfplay-hidden-weighted    3918969（9/19 21:32）             含む        600     8,250     1,855       340     1,309        70.6%        15.9%
  ika73/w12                   63081f9（9/22 19:53）             含む     43,999   592,200   133,596    24,121    94,441        70.7%        15.9%
  ika66/w24（課題文に無い）    df83b33（9/22 06:20）             含む     19,800   266,700    59,856    10,336    42,815        71.5%        16.1%
  ika66/w48（課題文に無い）    df83b33                           含む      8,400   113,461    25,342     4,441    18,150        71.6%        16.0%
  ika73/control, control-w16  63081f9                           含む     24 + 24  326 + 307  71 + 67   13 + 10   47 + 48
  g600（M-C 試走、dirty）      4dd5f46（9/24 05:44）             含む        600     6,855     1,746       200     1,233        70.6%        18.0%
  参考 selfplay-gen11h        53e2534（9/17 16:05）             含まない 12,000   147,167    36,900     4,179   （29,251）    —            0（下の null）
```

* どの教材も局は全部決着している（outcome が None の局 0）ので、学習の行 = 決定。
* gen11h の 29,251 は「同じ数えを当てたら」の仮の数。gen11h の交代の決定は本当の局面で解いていた（d959ab0 より前）ので、実際に動いた決定は 0（§2 の null で 279/279 が fixed と shifted でビット一致）。
* `selfplay-open2`（884ac5f、d959ab0 を含む）は公開情報の局で完成形を作らないので対象外。`selfplay-lead-gaendohido`（ff07f5d）は d959ab0 より前。
* `data/matches/` の 9/17 以後の控え隠蔽の対戦も両腕とも同じ誤りで打っている（教材ではないので数えていない。§6）。

### 2. 解き直し（`replacement_audit.py solve`、1 コア、教材ごとに 96 局ぶん・§1 の「効いた可能性」の決定だけ）

同じ記録の決定を今の port（この枝の build）で 2 通りに解き、記録と比べた。入力は記録から作り直す:
局面（記録）、選択肢（`switch_actions_after_faint` の文字列が記録と一致することを確かめた。不一致 0）、シート（M-B は選出解の book の
class_sets から foeSix の並びと foeTeam の 4 体が一致する 6 体、M-C はプールのチーム）、見せたもの（§1 と同じ時代の規則）、
控えの事前分布（hidden-weighted・w12・w24・w48 は book の `BenchPrior`、g600 は `selection-solved` の解と先発の条件付け、gen11L・hidden2・
hidden-fixed は一様 = 当時の生成）、葉（記録の `searchObjective`: gen11L は value-gen9、他は value-gen11L。CPU で読む）。

**誤りの効果 = fixed と shifted の差**（どちらも今の port・同じ入力なので、道具のずれを含まない）:

```
  教材              標本   |ΔsearchValue|                                 符号付き平均    |ΔfoeSearchValue|   方策が変わった決定
                           平均     中央値   p90      最大    >0.01          (shifted−fixed)  平均    >0.01       自席 TV>0 / 相手 TV>0
  gen11L            218    0.0169   2.6e-3   0.0524   0.166   88（40%）      −0.0023         0.0082    49         0 / 1
  hidden2           203    0.0139   3.7e-3   0.0432   0.102   71（35%）      +0.0090         0.0053    32         3 / 0
  hidden-fixed      223    0.0096   2.5e-5   0.0303   0.166   63（28%）      +0.0030         0.0060    42         0 / 1
  hidden-weighted   216    0.0080   1.3e-4   0.0237   0.224   37（17%）      +0.0023         0.0028    14         1 / 1
  w12               200    0.0072   3.0e-4   0.0180   0.167   28（14%）      +0.0047         0.0038    18         0 / 0
  w24               207    0.0059   2.7e-4   0.0154   0.111   28（14%）      +0.0036         0.0037    23         0 / 2
  w48               207    0.0081   3.1e-4   0.0282   0.128   34（16%）      +0.0048         0.0028    18         0 / 1
  g600              212    0.0037   4.8e-5   0.0097   0.071   21（10%）      +0.0019         0.0039    23         0 / 1
  計               1,686                                                                                          4 / 7（0.65%）
```

* 方策はほぼ動かない。交代の行列はたいてい純粋な均衡で、値が数ポイント動いても最善の交代は同じ。変わった 11 決定は全部 TV = 1（純粋な手が入れ替わる）
* 値は動く。記録の値は正しい値より平均で少し高い（M-B の控え隠蔽の 7 教材中 6 つで +0.002〜+0.009。相手の控えが出てこないセルが側 0 に有利）
* 完成形の信念の重みのうち、誤ったセルを含む完成形の重み（側 0 の行列の平均）: 一様の時代 0.40〜0.50、事前分布の時代 0.23〜0.26、g600 0.16
* 標本は交代の決定の 7 割（§1）から取っているので、交代の決定全体では上の割合に約 0.71 を掛ける

**正の対照（shifted が記録を再現する）**: fixed と shifted が 1e-6 より離れる決定のうち、記録が shifted と 1e-6 以内で一致したもの /
記録が fixed と一致したもの:

```
  gen11L 84 / 0（126 中）  hidden2 105 / 0（125）  hidden-fixed 89 / 0（114）  hidden-weighted 99 / 0（121）
  w12 90 / 0（114）        w24 99 / 0（117）        w48 87 / 0（111）          g600 78 / 0（119）
  （記録に近いのが shifted の決定: 117・122・109・112・105・110・101・110）
```

つまり記録の答えは今の port で「番号を 1 つ上げた」答えで、正しい答えではない。一致しない残り（2〜4 割）は下の「道具のずれ」の大きさで、
IKA-208 以後の port だけの規則や 9/17〜9/24 に入った規則の差（メイカー等）。

**null 対照**:

```
  今のコードで生成した局（この枝、seed 2231 の M-C 16 局・seed 2232 の M-B 24 局）を同じ道具で:
    記録 = fixed   M-C 47/47・M-B 75/75 決定がビット一致（値・相手の値・方策とも差 0）
    記録 = shifted  0 決定（値が動いた 16・26 決定はすべて fixed 側に一致）              ← 正の対照の鏡
  gen11h（d959ab0 より前、交代を本当の局面で解いた。--definition true、value-gen9、279 決定）:
    fixed = shifted 279/279 がビット一致（本当の局面では誤りは働かない）
    記録 対 今の port: |ΔsearchValue| 平均 0.0069・>1e-6 が 89/279・>0.01 が 58/279、方策 TV>0 が自席 5・相手 7
                       ← 9/17 の記録と今の port の間の「道具のずれ」（規則の追加・葉の CUDA/CPU）。誤りの効果とは別物
  --jobs: g600 72 決定・w12 200 決定とも --jobs 1 と --jobs 4 で行がバイト一致
```

道具のずれ（gen11h で平均 0.0069）は誤りの効果（0.004〜0.017）と同じ桁なので、**「記録 対 今の port」の差をそのまま誤りの大きさと読まないこと**。
上の表は fixed 対 shifted（同じ port）で測っている。

### 3. 学習が読む項目（コードで確かめた）

* **価値関数**（`tools/encode_dataset.py` → `value.Dataset`）: 行 = 記録の**全決定**（move・replacement・selfswitch、kind は 0 と 1 に畳む）、
  入力 = 決定の**本当の局面**、目標 = `outcome`（局の勝敗）。`searchValue` は `search_value`、`foeSearchValue` は `foe_search_value` として
  保存されるが、目標になるのは `--td-lambda` を付けた時だけ（`value.td_target`）。**影響を受けた教材で学習したモデルはすべて `td_lambda: 0.0`**
  （value-gen11L・value-h12・value-hidden24・ika73-armD・ika73-armDg の meta）。`searchValue` は meta の `search_auc` という比べ物にだけ使われた
* 行の重み: `value.train` は行を一様に引く（kind ごとの重みは無い）。だから「学習の重みに占める割合」= 行の割合 = §1 の右端（16〜18%）。
  value-gen11L の学習プール（`selfplay-pool11L-encoded.npz` = selfplay-all + gen9 + gen11L、923,708 行）では gen11L の 26,931 行 = **2.9%**。
  value-h12（hidden2 だけ）16.0%、value-hidden24（gen11L + hidden2）16.4%
* **方策**（`tools/policy_dataset.py`）: `kind == "move"` の決定だけを読む（137 行目）。交代の決定は読まない
* 選出解・book: 交代の決定は読まない
* **残る経路は 1 つ: 局の進み方。** 両席の交代はずれた方策から引かれた。§2 で方策が変わったのは効いた決定の 0.65%、効いた決定は 1 局に約 2.2 なので、
  **局の約 1.4% で、交代 1 回が別の分布から引かれた**（それでも引いた手が同じことはある）。その局の勝敗はその局を打ったエージェントの本当の勝敗で、
  目標としては正しい。違うのは「どのエージェントの局か」がわずかにずれることだけ

### 4. 結論: (a) 放置してよい。次の世代（M-C gen-0）は IKA-209 以後の port で作るので、何もしなくても消える

根拠:
1. 価値関数の目標（勝敗）と入力（本当の局面）は動いていない。記録の `searchValue` は `--td-lambda 0` の学習では読まれない（§3）
2. 方策の教材は交代の決定を読まない（§3）
3. 局の進み方への効きは、効いた決定の 0.65%（11/1,686）・局の約 1.4% に 1 回の交代だけ（§2・§3）
4. 捨てる・重みを下げる (b) は、目標が正しい行を捨てることになる（効いた行は学習の行の 16〜18%）。作り直す (c) は gen11L 規模で数時間の生成で、
   得るものが上の 1.4% の局の分布だけ

**守る条件（これを破るなら (b) にする）**:
* 影響を受けた教材（§1 の「含む」の行）で `--td-lambda > 0` を使う、または `searchValue`・`foeSearchValue` を目標や較正に使うなら、
  **`kind == replacement` の行を外す**（自交代は影響なし。ただし encode は自交代も kind 1 に畳むので、外すなら記録の `kind` で分ける）。
  効いた決定の値は平均 0.004〜0.017、最大 0.22 ずれ、向きもそろって高い（+0.002〜+0.009）
* 記録の交代の決定の `searchValue`・方策を読む分析（探索の楽観の測り・葉の較正など）を影響を受けた教材で回すなら、同じく交代の決定を外すか、
  この節の値を注記する

**M-C gen-0 について**: IKA-77（§5 の本番のコマンド）と IKA-81 の節には、**既存の教材（g600・w12 等）を M-C gen-0 の学習に混ぜる予定は書かれていない**。
M-C gen-0 は `data/selfplay-mc0` に 40,000 局を新しく生成し、IKA-86 の案は M-B のモデル（value-gen11L）からの温間始動（`--init-from`）。
* 新しい 40,000 局は IKA-209 以後の port で生成するので、この誤りは無い
* 生成の葉と温間の起点の value-gen11L は、学習の行の 2.9% が gen11L（影響あり）。ただし §3 のとおり目標は動いておらず、効くのは局の進み方の 1.4% だけ。起点として問題にしない
* もし g600 を混ぜる案が出たら: この誤りは混ぜない理由にならない（効いたのは記録の値だけ、目標は正しい）。**混ぜない理由は別にある**:
  g600 は 4dd5f46（dirty）の 600 局で、その後に入った M-C の規則（IKA-201 のメイカーのフィールド、IKA-202 のおうごんのからだ・フラワーベール 等）
  が無い局面で打っている。混ぜるなら規則の差を先に数えること。w12 は M-B で、M-B を M-C に使う道は温間始動（IKA-86）で決まっている

### 5. 深さ 2 の比べの数字（規則の差の注記）

IKA-209 §3 の指摘: 深さ 2 の枝（`search._refined_value`）は IKA-209 まで Python の `resolve_turn` で解かれ、深さ 1 の行列は橋（port）で埋めていた。
橋は 9/12 05:40（rustnode.py の追加、cd23aab）からあり、`generate_queue.py`・`match_queue.py` は既定で on にしていた（3222fde で確かめた）。
**ただし master の上で port と Python の規則が意図して違ったのは、IKA-208 の取り込み（59885d1、9/24 09:05: いのちがけ・トリック・だいばくはつ等）
から IKA-209 の取り込み（5d54631、9/24 12:22）までの約 3 時間 20 分だけ**で、それより前の port は Python と同じ規則を目指した移植（diff_node で一致を見ていた。port が断るセルは
Python で埋めた）。IKA-209 §3 で深さ 2 の決定を動かした唯一の規則の差（局面 302 のいのちがけ）は IKA-208 の port だけの規則。

記録の深さ 2 の比べと判断:

```
  TODO.md の見出し（行）                                      数字                                                   打った時期           注記
  G3（1149）・GENERATIONS の「深さ2 −1.2」                    depthmatch-d2-vs-d1                                    9/12 01:25          不要（橋より前。両方 Python）
  G35（2348）「真の深さ2」の値段                              225倍・78倍・約2.1倍                                    9/20                不要（時間の測定で、規則は読まない）
  9/20 IKA-12（5086・5126）                                   深さ2@24 対 深さ1@24 48.28% ±0.71（16,000局）          9/20                下の軽い注記
                                                              深さ2@24 対 深さ1@48 46.62% ±1.17、+0.00715 ±0.00095
  9/23 IKA-68（5579〜5819、GENERATIONS 1423〜1444）            セル単位 −0.00076 ±0.00656、制限深さ2 対 深さ1@24       9/23 朝             下の軽い注記
                                                              54.45% ±0.94（8,808局）、制限 対 混合 SPRT H1、
                                                              +0.00775 ±0.00307、turn-1 セル −0.00932 ±0.00601
  9/24 IKA-209 §3・IKA-209（続き）§4（diff_depth2）            400 局面で 386・352 一致                                —                   この数字自体が差の測定
```

**判断: いのちがけ（IKA-208 の規則の差）の注記はどの数字にも付けない。** 付けるのは IKA-12 と IKA-68 の数字への次の軽い注記だけで、
数字を疑う根拠ではない:

> 深さ 1 の行列は port、深さ 2 の枝は Python の resolver で解いた（同じ規則を意図した 2 つの実装。IKA-208 の port だけの規則より前）。
> セル単位の `depth2 − depth1` には、当時の 2 実装の食い違いが混ざり得る。その大きさは当時は測っていない
> （IKA-209 §3 の 9/24 の測りでは、規則以外の食い違いは 400 局面中 13 局面・値の差 1e-16 の桁）。

9/24 09:05〜12:22 の窓に打った深さ 2 の比べは TODO.md に無い（grep: depth・深さ2・深さ 2・depthmatch・ika68・depth2_cost）。

### 6. 別課題の候補

* `data/matches/` の 9/17（d959ab0）〜9/24（IKA-209）の控え隠蔽の対戦（ika66-*・ika73-*・ika122・ika141・ika143・h12-vs-o12-hidden 等）は、両腕の交代の
  決定が同じ誤りで解かれた。両腕同じ誤りなので勝率の比べには大きく効かないはず（方策の変化は効いた決定の 0.65%）だが、数えていない。要るなら
  この道具の `count` をそのまま当てられる（対戦の記録も同じ形）
* `--td-lambda` を将来使うときの「交代の決定を外す」旗（encode_dataset の `--kinds` は provenance の kind で、決定の kind では選べない）

### 7. 道具（`tools/replacement_audit.py`）

* `count <dir>...`: ファイルごとに 1 ワーカー（`--jobs`）。JSON だけを読む（port・葉なし）。`--carry slots|identities|recorded`・`--onboard species|shown` で時代を選ぶ
* `solve <dir>`: 決定ごとに 1 ワーカー（`--jobs`、各ワーカーが葉と port を持つ）。`fixed`・`shifted` と記録を並べ、`--out` に決定ごとの行、標準出力に集計。
  `--definition true` は d959ab0 より前の形（本当の局面の 1 行列）。`--prior none|book|pool`・`--leads`・`--book`・`--pool`・`--selection-store`
* null 対照（§2）: 今の局では記録 = fixed がビット一致、`--jobs 1` と `--jobs 4` の行がバイト一致
* ruff・`port_coverage.py --check`・`port_gate_audit.py --check`・tests/test_line_endings.py は通る

### 8. 機械（heavy.py、--agent IKA-223）

```
  14:17:20–14:17:47  cargo build --release              8 コア  27 s
  14:22:02–14:22:10  solve の試し（--jobs 1 と 4）       1・4 コア  5 s + 3 s
  14:22:53–14:23:55  今のコードの M-C 16 局（null 用）    1 コア  62 s
  14:24:34–14:25:19  今のコードの M-B 24 局（null 用）    1 コア  45 s
  14:25:54–14:26:10  count（全教材 約 14 GB）            8 コア  5 s + 11 s
  14:26:44–14:28:27  solve 11 本（教材 8・null 3）        1 コア  計 101 s（1 本 4〜14 s）
  14:31:56–14:32:12  test_line_endings 1 s・solve w12 --jobs 4 5 s・port_coverage 1 s・port_gate_audit 0 s（bash -c で包んで失敗した 1 行 0 s を含む）
