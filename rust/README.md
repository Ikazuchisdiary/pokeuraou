...
確保は **557.6 → 380.9 回/ターン（-32%）** まで落ちました。速い時は 26% 速い。
**ただし速度が二極化しました**——同じバイナリが、プロセスごとに 35 us か 87 us の
どちらかに落ち着きます（10 回中 4 回が速い側）。既存のビルドは 35 回測って
一度も遅い側に落ちていません。`Position` が 464 バイトでも 528 バイトでも起きるので、
サイズそのものではなくアドレス依存（ASLR とキャッシュの競合）に見えます。

**2.5 倍の地雷を踏むかもしれない変更は、半分の確率で速くても入れられません。**
入れていませんが、捨ててもいません——`rust/experiments/inline-position.patch` に
パッチと測定結果があります。アイドルなマシンで確かめるところからです。
