...
- **評価キャッシュ**は効きます——下の測定の通り。

### ノードの葉の 41.8% は「すでにある局面」

dlshogi がキャッシュする理由がここにもあるかを測りました。6 ノード 28,827 葉:
...
重複 1 つにつき、符号化 1 回・パイプ 3.7 KB・順伝播 1 行が無駄になっています。

**ただし入れていません。** 重複を落とすとバッチの行数が変わり、float32 の行列演算は
行数に依存するので、勝率の下位桁が動きます。**それはこの移植が守ってきた
「対戦が 1 バイトも変わらない」を壊します**——実際、拒否されたセルの葉が
別バッチで採点されるだけで探索値が 1e-8 動いたのを、このセッションで観測しています。

選択肢は 3 つで、**どれを取るかは移植の判断ではありません**:

1. 入れない（現状）。
2. Rust 側だけ重複を落とし、Python が順伝播の前に配列を展開し直す。
   ビット単位は保たれますが、17 MB の展開コピーが戻ってくるので**ほぼ相殺**です。
3. Python 側でも同じ重複排除をする。**約 10%** 速くなり、Python と Rust は一致したままですが、
   **以前に生成した対戦とは再現しなくなります**（勝率が 1e-7 ほど動くため）。
...
3. **葉の重複排除**（上の 41.8%）。幅 48 では符号化 16% + パイプ 3.5% + torch 10% に
   効くので、幅 24 のときより見返りが大きい——ただしビット単位の再現性と引き換えです。
