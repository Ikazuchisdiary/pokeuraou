...
1. **ワーカーを増やす**（ただ）。`tools/bench_workers.py` で実設定のまま測れます。
...
`tools/bench_workers.py` は N 個の生成プロセスを同時に起動して、
機械全体の games/min を返します。**ただし今回の測定中はこのマシンで
`tools/generation_match.py` が 11 プロセス動いていた**ので、下の曲線は
「その上にあと何個乗るか」です。空いているときに測り直してください。

```
workers   steady g/min   per worker   scaling
      1           46.1         46.1     100%
      2           88.8         44.4      96%
      4          155.3         38.8      84%
      8          251.8         31.5      68%
...
最初の 1 波は捨てます。冷えたファイルキャッシュとブーストしていない CPU で測ると
43% 遅く出て、**超線形スケーリングに見えます**。
